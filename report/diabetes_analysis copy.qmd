...
This project attempts to predict diabetes status using the Logistic Regression and LinearSVC models, against a baseline DummyClassifier on an imbalanced dataset. All models achieved similar accuracy on the test set (approximately `{python} round(test_score.iloc[0].mean(), 2)`), which highlights a key issue: accuracy alone is not a reliable performance metric (see @tbl-testing).

<!-- ![Accuracy of Logistic Regression and Linear SVC Model](../results/models/model_testing.png){#fig-model_testing width=60%} -->
```{python}
# | label: tbl-testing
# | tbl-cap: Accuracy of Logistic Regression and Linear SVC Model
# | echo: false

# =============================== for presenting table.head
Markdown(test_score.head().to_markdown(index=False))
```
...
For this analysis, we split the dataset into training `{python} f"{round(r_train * 100)}%"` and testing `{python} f"{round(r_test * 100)}%"` sets using a fixed random state (522) to ensure reproducibility. We implemented two classification algorithms:
...
Our results show that both models achieve approximately `{python} f"{round(np.mean(train_score.iloc[-2,1:]), 2) * 100}%"` accuracy, with logistic regression demonstrating slightly faster training time. 
...
The baseline DummyClassifier achieves an accuracy score of about `{python} round(train_score.iloc[-2,1],2)`, derived from assigning the most frequent class (non-diabetic) to all patients. This highlights how approximatey `{python} f"{round(train_score.iloc[-2,1],2) * 100}%"` of the dataset is non-diabetic. Both Logistic Regression and LinearSVC achieve similar accuracy (approximately `{python} round(np.mean(train_score.iloc[-2,2:]), 2)`) with little to no improvement (see @tbl-training).

<!-- ![Cross Validation Results of Candidate Models](../results/models/model_training.png){#fig-model_training width=60%} -->

```{python}
# | label: tbl-training
# | tbl-cap: Cross Validation Results of Candidate Models
# | echo: false

# =============================== for presenting table.head
Markdown(train_score.head().to_markdown(index=False))
```
